...
while still others can be a function of both to some degree
...
extention to stable population theory. Some might say that stable population
theory is not relevant to everyday demography, since populations are always changing. Even if change precludes the
...
rather than to a birth flow. The chronological-age classified renewal model and
the thanatological renewal model both describe the movement of population stocks
over time, but from opposite vantage points. Specifying a model of thanatological renewal expands the toolbox available to approach complex
...
\end{equation}
 ...
 \begin{equation}
  R_0 = \int _{y=0}^\infty \int _{a=0}^\infty d(a+y) \gamma(y) \dd a
\dd y
\end{equation}
