...
matrices are irreducible and primitive, and so will always have a
dominant positive real eigenvalue, the natural log of which is $r$. This
observation is sufficient to demonstrate the strong ergodic
theorem:\footnote{This step was taken for granted in jumping from
equation~\eqref{eq:ergostep1} to ~\eqref{eq:ergostep2}} i.e., given a fixed
matrix and enough time, the population will conform to some stable thanatological age distribution. The eigenvector corresponding to the dominant eigenvalue gives this stable thanatological age structure. The square
of the, or the pace of convergence to stability (the ratio of the \nth{1} to the \nth{2} eigenvalues).
