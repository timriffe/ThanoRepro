...
The
thanatological lifecycle graph is directed and strongly connected. The matrix is
also redudcible because each thanatological age class is reachable from every
other thanatological age class, either directly via fertility, or else by ageing toward lower ages. The thanatological matrix is also
easy to identify as primitive, since it contains self-loops (fertility elements
on the matrix diagonal). The matrix is said to be regular, since we can
imagine some value $k$ where each thanatological age class is $k$ steps away
from every other thanatological age class. In the fertility block of the matrix
(all rows, but not necessarily all columns), $k$ is 0, due to direct connections
between all fertile thanatological ages, but in order to make this condition hold for pre-menarcical females with very long lives, $k$ must be equal to the maximum lifespan. This can also be demonstrated simply by
setting $\textbf{Y}^k$ to some large value such that all elements of $\textbf{Y}
> 0$. 
